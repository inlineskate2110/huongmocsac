...
if( ! function_exists( 'mfn_styles' ) )
{
	function mfn_styles() 
	{
		...
		wp_enqueue_style( 'wfs-icons',			THEME_URI .'/fonts/aluent-icons.css', false, THEME_VERSION, 'all' );
		wp_enqueue_style( 'wfs-font',			THEME_URI .'/css/font-awesome.min.css', false, THEME_VERSION, 'all' );
		wp_enqueue_style( 'wfs-base',			THEME_URI .'/css/base.css', false, THEME_VERSION, 'all' );
		wp_enqueue_style( 'wfs-layout',			THEME_URI .'/css/layout.css', false, THEME_VERSION, 'all' );
		wp_enqueue_style( 'wfs-shortcodes',		THEME_URI .'/css/shortcodes.css', false, THEME_VERSION, 'all' );	
		
		// plugins
		if( ! isset( $theme_disable['entrance-animations'] ) ){
			wp_enqueue_style( 'wfs-animations',	THEME_URI .'/assets/animations/animations.min.css', false, THEME_VERSION, 'all' );
		}
	
		wp_enqueue_style( 'wfs-jquery-ui', 		THEME_URI .'/assets/ui/jquery.ui.all.css', false, THEME_VERSION, 'all' );
		wp_enqueue_style( 'wfs-prettyPhoto', 	THEME_URI .'/assets/prettyPhoto/prettyPhoto.css', false, THEME_VERSION, 'all' );
		wp_enqueue_style( 'wfs-jplayer',		THEME_URI .'/assets/jplayer/css/jplayer.blue.monday.css', false, THEME_VERSION, 'all' );	
		
		// rtl | demo -----
		if( $_GET && key_exists('wfs-rtl',$_GET) ) wp_enqueue_style( 'mfn-rtl', THEME_URI .'/rtl.css', false, THEME_VERSION, 'all' );
		
		// Responsive -------------------------------------------------------------
		if( mfn_opts_get('responsive') ){
			wp_enqueue_style( 'wfs-responsive', THEME_URI .'/css/responsive.css', false, THEME_VERSION, 'all' );
		} else {
			wp_enqueue_style( 'wfs-responsive-off', THEME_URI .'/css/responsive-off.css', false, THEME_VERSION, 'all' );
		}
		...
		if( mfn_opts_get( 'static-css' ) && ( ! ( $_GET && key_exists('mfn-c',$_GET) ) ) ){	
			...
			if( defined( 'STATIC_IN_CHILD' ) && STATIC_IN_CHILD ){
				wp_enqueue_style( 'wfs-style-static', CHILD_THEME_URI .'/style-static.css', false, THEME_VERSION, 'all' );
			} else {
				wp_enqueue_style( 'wfs-style-static', THEME_URI .'/style-static.css', false, THEME_VERSION, 'all' );
			}
			...
			if( $skin != 'custom' && $skin != 'one' ){
				...
				// Predefined Skins
				wp_enqueue_style( 'wfs-skin-'. $skin, THEME_URI .'/css/skins/'. $skin .'/style.css', false, THEME_VERSION, 'all' );
...
// 			echo $css;
			...
			wp_enqueue_style( 'wfs-forge', THEME_URI .'/css/forge.css', false, THEME_VERSION, 'all' );
		echo '</style>'."\n";
	}
}
