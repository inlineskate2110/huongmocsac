...
function disable_plugin_updates( $value ) {
	unset( $value->response['loco-translate/loco-translate.php'] );
	unset( $value->response['woocommerce/woocommerce.php'] );
	unset( $value->response['jetpack/jetpack.php'] );
	unset( $value->response['litespeed-cache/litespeed-cache.php'] );
	return $value;
}
	...
	if ( !function_exists( 'alpha_store_cart_link' ) ) {
		...
		function alpha_store_cart_link() {
			...
			<a class="cart-contents text-right" href="<?php echo esc_url( WC()->cart->get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'alpha-store' ); ?>">
				<i class="fa fa-shopping-cart"><span class="count"><?php echo absint( WC()->cart->get_cart_contents_count() ); ?></span></i><div class="amount-title"><?php echo esc_html_e( 'Giỏ hàng ', 'alpha-store' ); ?></div><div class="amount-cart"><?php echo wp_kses_data( WC()->cart->get_cart_subtotal() ); ?></div>
			</a>
			<?php
		}
	...
	function alpha_store_wishlist_products() {
		if ( function_exists( 'YITH_WCWL' ) ) {
			...
			<div class="add-to-wishlist-custom add-to-wishlist-<?php echo esc_attr( $id ); ?>">
				<div class="yith-wcwl-add-button show" style="display:block" data-toggle="tooltip" data-placement="top" title="<?php esc_attr_e( 'Yêu thích', 'alpha-store' ); ?>"> <a href="<?php echo esc_url( $url ); ?>" rel="nofollow" data-product-id="<?php echo esc_attr( $id ); ?>" data-product-type="simple" class="add_to_wishlist"></a><img src="<?php echo get_template_directory_uri() . '/img/loading.gif'; ?>" class="ajax-loading" alt="loading" width="16" height="16"></div>
				<div class="yith-wcwl-wishlistaddedbrowse hide" style="display:none;"> <span class="feedback"><?php esc_html_e( 'Added!', 'alpha-store' ); ?></span> <a href="<?php echo esc_url( $wishlist_url ); ?>"><?php esc_html_e( 'Sản phẩm đã thích', 'alpha-store' ); ?></a></div>
